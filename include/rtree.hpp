...
public:
  ...
  bool Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::vector<DATATYPE>& result);

  bool KNN(const ELEMTYPE coord[NUMDIMS], const int k_value, std::vector<DATATYPE>& result, ELEMTYPEREAL dis_limit = -1, t_resultCallback predicate = nullptr, void* a_context = nullptr);
...
protected:
  ...
  {
    Branch* m_branch;
    ELEMTYPEREAL m_distance;

    KNNode(Branch* branch, ELEMTYPEREAL dis) {
      m_branch = branch;
      m_distance = dis;
    }
    ...
    bool operator < (const KNNode& other) const
    {
  ...
  // struct knnode_greater_cmp
  // {
  //     bool operator()(const KNNode& left, const KNNode& right)
  //     {
  //         return left.m_distance > right.m_distance;
  //     }
  // };
  // struct knnode_less_cmp
  // {
  //     bool operator()(const KNNode& left, const KNNode& right)
  //     {
  //         return left.m_distance < right.m_distance;
  ...
  ELEMTYPEREAL CalcAxisDis(const ELEMTYPE value, const ELEMTYPE min, const ELEMTYPE max);
  ELEMTYPEREAL CalcPointRectDis(const ELEMTYPE coord[NUMDIMS], const Rect* a_rect);
  ...
  bool Search(Node* a_node, Rect* a_rect, std::vector<DATATYPE>& result);
...
class RTFileStream : public std::fstream
{
public:
  template< typename TYPE >
  void Write(const TYPE& a_value)
  {
    write((const char*)&a_value, sizeof(a_value));
  }

  template< typename TYPE >
  void WriteArray(const TYPE* a_array, int a_count)
  {
    write((const char*)a_array, sizeof(TYPE) * a_count);
  }

  template< typename TYPE >
  void Read(TYPE& a_value)
  {
    read((const char*)&a_value, sizeof(a_value));
  }

  template< typename TYPE >
  void ReadArray(TYPE* a_array, int a_count)
  {
    read((const char*)a_array, sizeof(TYPE) * a_count);
  }
};
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::vector<DATATYPE>& result)
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  return Search(m_root, &rect, result);
...
bool RTREE_QUAL::KNN(const ELEMTYPE coord[NUMDIMS], const int k_value, std::vector<DATATYPE>& result, ELEMTYPEREAL dis_limit, t_resultCallback predicate, void* a_context)
{
    ...
    while (node) 
    {
        ...
        {
          node = knnode_queue.top().m_branch->m_child;
          knnode_queue.pop();
        }
        else
        {
...
bool RTREE_QUAL::Load(const char* a_fileName)
{
  ...
  RTFileStream stream;
  stream.open(a_fileName, std::ios::in|std::ios::binary);
  if (!stream.is_open())
  {
    return false;
  }
...
{
  RTFileStream stream;
  stream.open(a_fileName, std::ios::out|std::ios::binary);
  if(!stream.is_open())
  {
    return false;
  }
...
RTREE_TEMPLATE
ELEMTYPEREAL RTREE_QUAL::CalcAxisDis(const ELEMTYPE value, const ELEMTYPE min, const ELEMTYPE max)
{
    if (value < min)
      return (ELEMTYPEREAL)(min - value);
    if (value > max)
      return (ELEMTYPEREAL)(value - max);
    return (ELEMTYPEREAL)0;
}


RTREE_TEMPLATE
ELEMTYPEREAL RTREE_QUAL::CalcPointRectDis(const ELEMTYPE coord[NUMDIMS], const Rect* a_rect)
{
  ELEMTYPEREAL distance = (ELEMTYPEREAL)0;
  for(int axis=0; axis<NUMDIMS; ++axis)
  {
    distance += CalcAxisDis(coord[axis], a_rect->m_min[axis], a_rect->m_max[axis]);
  }
  return distance;
}
...
// Search in an index tree or subtree for all data retangles that overlap the argument rectangle.
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, std::vector<DATATYPE>& result)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search(a_node->m_branch[index].m_child, a_rect, result))
        {
          return false;
        }
      }
...
typedef RTree<double, int, 2, double> RTree2d;
